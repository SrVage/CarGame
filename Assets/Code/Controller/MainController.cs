...
namespace Code.InputLogic
{
    public class MainController:BaseController
    {
        ...
        public MainController(ProfilePlayer model, Transform uiRoot, List<ShopItem> shopItems, UpgradeItemRepository item)
        {
            _model = model;
            _uiRoot = uiRoot;
        ...
        private void OnStateChange(GameState state)
        {
            switch (state)
            {
                ...
                case GameState.Start:
                    ...
                    _menuController = new MainMenuController(_uiRoot, _model, _shopItems, _upgradeItemRepository);
                    //_inventoryController = new InventoryController(_inventoryModelModel, _itemsRepository);
                    break;
                case GameState.Game:
                    _menuController?.Dispose();
                    _menuController = null;
                    _gameController = new GameController(_model, _inventoryController);
                    break;
                default:
